...
Several actuator-level control architectures have been used by the literature. The choice is not neutral. Peng and van de Panne compared torque, position-target, velocity and muscle-activation action spaces on simulated planar characters, and found that the spaces carrying low-level feedback learn faster, reach better final performance, and tolerate lower policy rates~\cite{peng2017actionspace}. Their study is in simulation and planar, so it bounds expectations rather than settling the question, and the authors themselves note that impedance-style tasks may favour the lower-level spaces.

\textbf{Direct torque control} provides the policy with the highest level of control authority, but also imposes the most demanding communication and control-loop requirements. It is the mode the quasi-direct-drive actuator lineage was designed for~\cite{seok2015design,wensing2017proprioceptive}, and the one MIT Cheetah commands through~\cite{dicarlo2018mpc}. It was also the weakest of the four action spaces in that comparison, and the one that degraded fastest as the policy rate dropped~\cite{peng2017actionspace}. In simulation, the policy can directly command joint torques at a high frequency. On the physical robot, however, closing such a fast loop requires the policy, communication bus, actuator controller, and state estimation to operate with sufficiently low latency. In the present hardware architecture, this approach was not practical through the CAN communication bus at the required frequency.
...
\textbf{Impedance control} provides an intermediate level of control authority. Instead of directly specifying the actuator torque or being restricted to only position, the policy can influence the desired mechanical behaviour through position, stiffness, damping, and/or torque-related parameters. The $[q_d, \dot q_d, K_p, K_d, \tau_{\text{ff}}]$ command frame established by the Cheetah lineage is the standard realisation~\cite{katz2018actuator,katz2019minicheetah}, and \cref{eq:impedance} is the form this thesis uses. It is also the ancestor of the CAN protocol the CubeMars drives accept, which is why the mode is available on this robot at all. Learning the compliance terms rather than fixing them is established on manipulators and demonstrated on a legged hopper in hardware~\cite{bogdanovic2020impedance}, but I have found no prior work that learns the full five-term frame on a walking robot.
...
The second major design dimension is the amount and location of prior structure imposed on the gait. The families below are ordered by how much they constrain the gait. No published study benchmarks them against each other under a common protocol: the field's synthesis is narrative~\cite{ha2025survey}, and the head-to-head comparisons that exist sit inside method papers and cover two arms at a time~\cite{cpgrl2022}. The ordering is therefore mine, and it ranks constraint, not performance.
