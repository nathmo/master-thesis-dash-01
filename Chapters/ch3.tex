...
At 50\,Hz the mean period overshoots its 20\,ms target and the jitter is an order of magnitude
above the faster rates. The likely cause is the non-real-time scheduler, which wakes the loop late
after the longer sleeps.

\subsubsection{Actuator thermal characterisation}

Not measured due to lack of time. The thermal lumped model parameters were estimated roughly from the mass of the actuator but not used in the simulation.
...
A systemd entry starts the web interface at boot. The Raspberry Pi acts as a WiFi access point (SSID "runningmachine", password "runningmachine").
...
The latency-included rail model also gives the sensitivity of the bound to the design.
Four parameters were each scaled by $\pm30\,\%$ with everything else nominal: leg mass, rotor
inertia, deliverable peak torque and bus voltage (through the no-load speed).
Peak torque is the strongest lever: 30\,\% more raises the bound by about 1\,m/s.
Leg mass is second: 30\,\% less raises the bound by about 0.7\,m/s.
Rotor inertia is nearly flat, and 30\,\% more bus voltage gains nothing, because the bound does not
sit on the no-load-speed ceiling at this operating point.
Leg lenght was not simulated due to lack of time as it imply to update the URDF but it is assumed that longer leg would help increase the stride lenght.
The levers for a faster machine are therefore deliverable torque and leg mass.
Leg lenght is also likely a significant level but its impact and direction is unknown.
